...
Пример ниже использует сервер `158.160.79.193`, пользователя `dilukhin` и ключ `id_yandex_vm`.

### Инициализация сервера с AmneziaWG

```bash
python vcli-admin.py --host 158.160.79.193 --user dilukhin --auth key --key id_yandex_vm init
```

### Инициализация сервера со стандартным WireGuard

```bash
python vcli-admin.py --host 158.160.79.193 --user dilukhin --auth key --key id_yandex_vm init --no-amnezia
```

### Проверка состояния

```bash
python vcli-admin.py --host 158.160.79.193 --user dilukhin --auth key --key id_yandex_vm status
python vcli-admin.py --host 158.160.79.193 --user dilukhin --auth key --key id_yandex_vm health
```

### Создание пользователя без интернета

```bash
python vcli-admin.py --host 158.160.79.193 --user dilukhin --auth key --key id_yandex_vm add user1
```

### Создание пользователя с интернетом

```bash
python vcli-admin.py --host 158.160.79.193 --user dilukhin --auth key --key id_yandex_vm add user1 --internet
```

### Создание администратора

```bash
python vcli-admin.py --host 158.160.79.193 --user dilukhin --auth key --key id_yandex_vm add user1 --admin
```
...
```bash
python vcli-admin.py --host 158.160.79.193 --user dilukhin --auth key --key id_yandex_vm config user1
```
...
Команда `config` получает конфиг через серверный скрипт, а не прямым `scp` к файлу. Это нужно потому, что конфиги на сервере хранятся с закрытыми правами и могут быть недоступны обычному SSH-пользователю.
...
- копирует `vsrv-admin.py` на сервер;
...
`/opt/vpn-admin` удаляется последним действием. Серверный скрипт удаляет сам себя вместе с каталогом, это ожидаемое поведение.
...
В версии `0.0.8` дублирование потокового вывода клиентом исправлено. Если дублирование осталось, нужно убедиться, что на клиенте и сервере используются файлы одной версии.
...
- Команда `config` отдаёт файл через серверный скрипт, чтобы не открывать права на чтение обычному SSH-пользователю.
