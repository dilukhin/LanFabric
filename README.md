...
Текущее состояние испытаний и незавершённых задач: [docs/current_status_ru.md](docs/current_status_ru.md). Согласованный порядок работ и зависимости автозапуска AWG: [docs/work_plan_ru.md](docs/work_plan_ru.md).
...
Проверенный на `dilyavm` развёрнутый baseline пока остаётся `0.0.17`. `0.0.18` — кандидат текущей реализации до локальной и стендовой приёмки.
...
На сервере full-tunnel по-прежнему имеет canonical `AllowedIPs = 0.0.0.0/0`. На Windows команда `config` локально адаптирует сохранённый конфиг к `AllowedIPs = 0.0.0.0/1, 128.0.0.0/1`: это сохраняет full-tunnel, но не включает kill-switch AmneziaWG/WireGuard и оставляет управленческий direct path к публичному Endpoint. Маршрут `/32` к Endpoint через обычный LAN gateway всё равно обязателен; команда автоматически проверит и добавит его, при необходимости показав UAC-запрос.
...
Зачем нужен маршрут: он сохраняет direct path к Endpoint через обычный LAN gateway. Без отдельного маршрута попытка связаться с VPN-сервером может уйти в ещё неработающий туннель. Windows-адаптация `/1 + /1` дополнительно избегает restrictive kill-switch, который включает literal `/0` в Windows-клиенте.
...
Для `awg` команда сначала строго проверяет уже существующие backend, серверную пару ключей, AWG-параметры и SQLite. Потерянные данные при восстановлении не создаются заново. Затем команда под общей межпроцессной блокировкой:

- закрывает собственный защитный firewall-барьер LanFabric;
- проверяет принадлежность существующего `wg0` либо создаёт `wg0 type amneziawg`;
- применяет конфигурацию, адрес и точный состав активных peers из одного снимка БД;
- пересобирает собственные цепочки доступа/NAT;
- проверяет обязательные runtime-инварианты при закрытом барьере;
- открывает трафик только после успешной проверки;
- сохраняет правила.

При ошибке барьер остаётся закрытым; silent fallback `awg -> wg` отсутствует. Повторный `start` может дать краткий перерыв уже подключённым клиентам.

Для `wg` сохраняется путь `wg-quick@wg0`; отдельное полное восстановление WG после boot относится к issue #19.

Ручной `start` не включает и не отключает persistent autostart.
...
Удаляет подтверждённо принадлежащий LanFabric runtime и его правила, но сохраняет БД, backend, ключи и конфиги. Для AWG команда не трогает чужой/неидентифицированный `wg0`.

`stop` не меняет persistent autostart: если AWG autostart включён, следующий обычный boot снова восстановит VPN.
...
Эквивалентно последовательному `stop` + `start` под одной серверной блокировкой. Возможен краткий перерыв соединений.

### autostart

Явно управляет восстановлением AWG при загрузке сервера:

```bash
python vcli-admin.py --host 198.51.100.42 --user donpedro autostart status
python vcli-admin.py --host 198.51.100.42 --user donpedro autostart enable
python vcli-admin.py --host 198.51.100.42 --user donpedro autostart disable
```

Контракт:

- новая успешная `init` с backend `awg` устанавливает и включает `lanfabric-awg.service`;
- unit — загрузочная задача `Type=oneshot`, а не постоянно работающая VPN-служба;
- после успешного выполнения unit может быть `inactive/dead`; состояние VPN определяется `status`/`health`, а не `systemctl is-active`;
- `enable` проверяет сохранённое состояние и доверенную root-границу, устанавливает unit и разрешает его, но не перезапускает текущий VPN;
- `disable` запрещает будущий boot restore, но не останавливает текущий VPN;
- обычные `start`/`stop`/`restart` не меняют enable/disable;
- `remove`/`purge` удаляют AWG autostart до удаления runtime или пакетов.

Миграция существующего сервера `0.0.17 -> 0.0.18` выполняется явно:

```bash
python vcli-admin.py --host 198.51.100.42 --user donpedro patch
python vcli-admin.py --host 198.51.100.42 --user donpedro autostart enable
python vcli-admin.py --host 198.51.100.42 --user donpedro start
python vcli-admin.py --host 198.51.100.42 --user donpedro health
```

`patch` сам по себе не включает autostart. Явный `start` после миграции нужен для применения нового fail-closed runtime-контракта к уже работающему AWG и может кратко прервать клиентские соединения.
...
### VPS был остановлен или перезагружен

Если AWG autostart включён, после обычной загрузки ручной `start` не нужен:

```bash
python vcli-admin.py --host <сервер> autostart status
python vcli-admin.py --host <сервер> status
python vcli-admin.py --host <сервер> health
```

Если autostart намеренно отключён, runtime можно поднять вручную командой `start`; это не изменит persistent-настройку autostart.

После cloud stop/start публичный IP сервера может измениться. Autostart восстанавливает серверный runtime, но не обновляет Endpoint в уже выданных клиентских конфигурациях.
...
- серверный модуль загружается во временный файл, проверяется и атомарно устанавливается в root-owned `/opt/vpn-admin`;
- БД, AWG-параметры, приватные ключи и клиентские конфиги хранятся с закрытыми правами;
...
- AWG autostart решает восстановление при boot, но не является непрерывным watchdog: удаление/падение `wg0` после успешной загрузки само по себе не перезапускает runtime.
...
Текущая очередь определяется [согласованным планом](docs/work_plan_ru.md): #17 → #15 → необходимая часть #16 → автозапуск #12 → локальная и cold-boot приёмка; затем Android, оставшиеся задачи безопасности, восстановление wg и исследование P2P. Результаты инспекции: [обзор от 2026-09-18](docs/reviews/LanFabric_project_inspection_2026-09-18.md). В текущем change set реализуется кандидат 0.0.18; проверенный стендовый baseline до отдельной приёмки остаётся 0.0.17.

Ниже — возможные последующие улучшения, а не утверждённый порядок реализации:
