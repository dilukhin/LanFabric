...
| Клиент первого прохода | локальный компьютер `DIMA-HP`, Windows |
| Топология первого прохода | один физический клиент, конфиги `alice` и `bob` проверяются последовательно |
...
9. `alice` и `bob` в обязательном проходе являются двумя VPN-пользователями,
   но не двумя физическими клиентскими узлами. Оба конфига проверяются по очереди
   на `DIMA-HP`; одновременно эти туннели не включать.
...
Для обязательного первого прохода достаточно одного физического клиентского
узла `DIMA-HP`. Одновременная работа нескольких устройств проверяется отдельным
будущим этапом и не влияет на приёмку первого прохода.
...
В обязательном первом проходе единственный физический клиентский узел —
локальный компьютер `DIMA-HP`. Конфиги `alice` и `bob` импортируются и
проверяются на нём последовательно. Это проверяет две политики доступа, но не
является тестом двух одновременно подключённых устройств.
...
### 11.3 Будущий многоклиентский проход

Этот этап не входит в обязательный первый проход. Его выполнять отдельно после
успешной проверки на `DIMA-HP`, с повторным recovery gate и отдельным
подтверждением пользователя.

Планируемые физические клиентские узлы:

- `DIMA-HP` — основной Windows-клиент;
- `DESKTOP-M22DPBH` — второй Windows-ноутбук;
- Android-телефон с клиентом AmneziaWG;
- при необходимости отдельный Linux-клиент.

Для каждого устройства создать отдельного тестового VPN-пользователя и отдельный
конфиг. Не копировать один приватный ключ на несколько устройств. Имена
пользователей, состав устройств и политики доступа зафиксировать перед началом
прохода.
...
Результаты многоклиентского прохода сохранять в отдельный отчёт. Не считать
последовательную проверку нескольких конфигов на одном `DIMA-HP` заменой этому
этапу.
