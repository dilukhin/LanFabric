...
План хранит принятый baseline LanFabric `0.0.17` и сценарий приёмки кандидата
`0.0.18` на существующей виртуальной машине Yandex Cloud. Основной проход проверяет AmneziaWG, SSH/SCP,
...
## 1.1 Актуальность после инспекции 2026-09-18

Принятые deployment/ручное восстановление AWG/двухклиентский Windows E2E записаны в [docs/current_status_ru.md](docs/current_status_ru.md). Их не повторять автоматически. Очередь работ и точка продолжения: [docs/work_plan_ru.md](docs/work_plan_ru.md).

Код 0.0.17 имеет подтверждённые ограничения: health может вернуть 0 при обнаруженных неисправностях; диагностика создаёт отсутствующую БД. Эти факты относятся к развёрнутому baseline.

Кандидат 0.0.18 должен доказать обратное локальными regression-тестами: diagnostics не создают данные, health даёт ненулевой exit при обязательном нарушении, AWG apply остаётся fail-closed при частичном отказе, операции сериализованы, root entrypoint защищён и autostart управляется явно.

Разделы ниже сохраняют исходные сценарии 0.0.17. Приёмка автозапуска по #12 выделена в разделе 13.1 и выполняется только после локального PASS, отдельного deployment task и отдельного cloud gate.
...
Значения в таблице — исходные данные предыдущего тестового цикла, а не
доказательство текущего состояния Yandex Cloud. Перед каждым новым проходом
необходимо перечитать фактические VM ID/state, публичный IP, загрузочный диск,
сеть/security groups, Backup ID/status/size и привязки backup policy. При
расхождении использовать фактическое состояние и остановить mutation path до
review, если расхождение неожиданное.

Публичный адрес прерываемой ВМ может измениться между проходами. В командах ниже
старое значение используется только как ориентир до read-only preflight.
...
9. Первый обязательный VPN-проход выполняется на одном физическом Windows-клиенте
   `DIMA-HP`: конфиги `alice` и `bob` проверяются последовательно, одновременно
   эти туннели не включаются. Реальный многоклиентский проход выполняется отдельно.

## 4. Критерии завершения

Обязательный проход считается успешным, если:

- локальные проверки завершились с 0 failures, 0 errors, 0 expected failures и
...
Первый обязательный проход не доказывает одновременную работу нескольких
физических VPN-клиентов. Эта проверка имеет отдельный gate после успешного
single-device прохода.
...
Значения `$HostIp`, `$VmId` и `$BackupId` считать предварительными до read-only
preflight. После проверки обновить переменные фактическими значениями.
...
- для кандидата: `vcli-admin 0.0.18`;
- для кандидата: `vsrv-admin 0.0.18`;
- до deployment фактический remote baseline `dilyavm` остаётся `0.0.17`;
- failures: 0;
- errors: 0;
- expected failures: 0;
...
Любой expected failure теперь является отклонением от текущего локального
baseline и блокирует переход к E2E до review.

Сохранить вывод локальных проверок в отчёт тестового прохода.

## 8. Этап 2. Облачный preflight

До любых cloud mutations сначала выполнить read-only preflight и зафиксировать
фактические VM state, public IP, disk/network state, recovery backup и backup
policy bindings. Старые значения из раздела 2 не являются основанием для
mutation.

### 8.1 Запустить ВМ

Если read-only preflight показывает, что ВМ выключена, остановить проход и
получить отдельное подтверждение на запуск. Запуск изменяет состояние и
тарификацию.
...
Этот пункт выполняется только если ВМ уже запущена либо её запуск был отдельно
разрешён и успешно завершён.
...
- серверный модуль имеет версию `0.0.17`.
...
В обязательном первом проходе используется один физический Windows-клиент —
`DIMA-HP`. Конфиги `alice` и `bob` импортируются и проверяются на нём
последовательно. Одновременно оба туннеля не включать. Такой проход проверяет
две политики доступа, но не является тестом двух одновременно подключённых
...
- выключить туннель перед проверкой `bob`.
...
### 11.3 Отдельный будущий многоклиентский проход

Этот этап не входит в обязательный первый проход. Его выполнять только после
успешного single-device E2E, с новым preflight/recovery gate и отдельным
подтверждением требуемых mutations.

Планируемые физические клиентские узлы:

- `DIMA-HP` — основной Windows-клиент;
- `DESKTOP-M22DPBH` — второй Windows-клиент;
- Android-телефон с клиентом AmneziaWG;
- при необходимости отдельный Linux-клиент.

Для каждого физического устройства создать отдельного VPN-пользователя и
отдельный конфиг. Не копировать один приватный ключ на несколько устройств.
Перед проходом зафиксировать состав устройств, имена пользователей и политики.

Проверить:

- одновременный handshake всех участвующих устройств;
- уникальные VPN-адреса и ключи;
- доступ каждого узла к `10.8.0.1`;
- разрешённую политикой доступность или изоляцию клиентов друг от друга;
- одновременную работу клиента без интернета и full-tunnel клиента;
- внешний IP full-tunnel узлов;
- блокировку одного пользователя без разрыва соединений остальных;
- повторное подключение после смены сети, если тестируется мобильный клиент;
- согласованность `list`, `status`, `health`, runtime и iptables на сервере;
- восстановление обычной маршрутизации каждого устройства после отключения VPN.

Результаты многоклиентского прохода сохранять в отдельный отчёт. Последовательная
проверка нескольких конфигов на одном `DIMA-HP` его не заменяет.
...
Сценарий ручного восстановления 0.0.17 после прерывания ВМ (исторический путь; не приёмка автозапуска #12):
...
## 13.1 Приёмка автозапуска AWG по #12 после реализации

Статус: **кандидат 0.0.18 реализуется, стендовая приёмка не выполнена**. Архитектурные требования: [замечания C1–C8](docs/reviews/LanFabric_awg_lifecycle_corrections_2026-09-18.md).

До стенда обязательны локальные проверки отсутствующих и повреждённых данных, промежуточных отказов, timeout/сигналов, ошибки удаления интерфейса, повторных действий и конкурентных изменений. Проверки с подменёнными командами не заменяют реальный трафик.

Deployment migration 0.0.17 → 0.0.18 выполняется отдельно от cold boot:

1. Подтвердить local PASS и clean expected branch/HEAD.
2. Проверить remote baseline 0.0.17, backend=awg, независимый SSH и безопасный снимок users/policies/keys/params без публикации секретов.
3. Выполнить штатный `patch`; убедиться, что серверный модуль установлен атомарно и имеет root-owned границу.
4. Явно выполнить `autostart enable`; проверить `autostart status`. Сам `patch` не включает unit.
5. Явно выполнить `start`, принимая краткий перерыв VPN, затем `status` и строгий `health`. Это мигрирует текущий AWG runtime на fail-closed firewall contract; persistent autostart от `start` не меняется.
6. Сравнить данные до/после. Не выполнять `init`.

После отдельного review deployment evidence — cold-boot проход:

1. Перечитать текущие VM ID/state/IP, путь независимого SSH-доступа и требуемое recovery-состояние. dilyavm не останавливать без отдельного разрешения на этот проход.
2. Проверить фактическую версию 0.0.18 и `autostart status`: unit установлен и enabled. Сохранить сравнимый снимок пользователей, ключей, AWG params, backend и политик без публикации секретов.
3. Выполнить согласованные остановку/запуск exact VM через штатный yc/guard; разрешение на конкретные операции получить по YC policy.
4. Перечитать публичный IP. При его изменении безопасно обновить клиентский Endpoint и отметить это отдельно.
5. **Не выполнять vcli start или sync вручную.** В пределах согласованного конечного срока проверить результат загрузочной задачи и фактический runtime.
6. Подтвердить backend awg, интерфейс нужного типа в UP с адресом 10.8.0.1/24, правильный UDP-порт, IPv4 forwarding, состав peers/AllowedIPs, отсутствие blocked peers, правила доступа/NAT.
7. Подтвердить неизменность снимка данных, handshake и полезный трафик клиента; для интернет-политик проверить разрешённый и запрещённый выход через VPN-сервер.
8. Проверить согласованный результат status/health, независимый SSH и итоговое состояние стенда.

Успех boot-only oneshot не означает `active` после завершения: штатное состояние unit после успешного запуска может быть `inactive/dead`. Проверяются `Result=success`, enablement и фактическая готовность runtime. Наличие одного wg0 или локально слушающий порт по отдельности не закрывают #12.

Реальные failure-injection/remove/purge/смена backend — отдельные разрешённые задачи; не расширять ими cold-boot проход без решения. Финальное состояние VM и клиентов зафиксировать в отчёте, VM автоматически не останавливать.
...
Выполнять только при сохранённом независимом доступе по `id_yandex_vm` и как
отдельный security-sensitive task.

Локальные regression-тесты текущего `master` уже закрывают ранее известные
дефекты порядка `untrust`, очистки stale temporary sudoers и дублирования
permanent trust. Реальный E2E должен подтвердить эти инварианты на тестовой ВМ,
но не считать их текущими известными дефектами.

Оставшиеся отдельные вопросы для дальнейшего security review:
...
4. Повторить `trust TRUST` тем же локальным ключом и убедиться, что остаётся
   ровно одна LanFabric-owned permanent строка этого ключа; foreign и temporary
   строки не должны удаляться.
5. Выполнить `untrust` и проверить, что sudo trust отозван до удаления SSH trust.
6. Проверить отсутствие целевого LanFabric-маркера и соответствующего sudoers.
7. Проверить адресную очистку stale temporary sudoers без расширения NOPASSWD
   allowlist.
8. Повторно проверить независимый доступ по `id_yandex_vm`.
