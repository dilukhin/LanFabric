...
План предназначен для ручной проверки LanFabric `0.0.17` на существующей
...
Код 0.0.17 имеет подтверждённые ограничения: health может вернуть 0 при обнаруженных неисправностях; диагностика создаёт отсутствующую БД. Поэтому прежние указания status/health ниже требуют анализа результата и дополнительных независимых проверок, а не одного exit code. Известные проблемы не считаются исправленными публикацией плана.

Разделы ниже описывают исходные сценарии 0.0.17. Новая приёмка автозапуска по #12 выделена в разделе 13.1 и выполняется только после реализации, проверки и отдельной задачи на стенд.
...
Статус: **запланировано, не выполнено**. Архитектурные требования: [замечания C1–C8](docs/reviews/LanFabric_awg_lifecycle_corrections_2026-09-18.md). Предусловия: проверены #17, #15 и обязательная часть #16; согласованы выпуск/миграция/семантика stop.

Сначала локально проверить отсутствующие и повреждённые данные, промежуточные отказы, timeout/сигналы, ошибку удаления интерфейса, повторные действия и конкурентные изменения. Проверки с подменёнными командами не заменяют реальный трафик.

Отдельный стендовый проход:
...
2. Проверить фактическую версию и установленный автозапуск. Сохранить сравнимый снимок пользователей, ключей, AWG params, backend и политик без публикации секретов.
...
Успех oneshot не обязан означать active unit после завершения; критерии зависят от принятого контракта. Наличие одного wg0, нулевой exit code старого health или локально слушающий порт по отдельности не закрывают #12.
