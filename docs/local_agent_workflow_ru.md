...
## Подтверждение пользователем и исполнение агентом

Для служебных safety/guard-команд, где требуется явное подтверждение человека:

1. агент сам готовит точную команду;
2. показывает пользователю:
   - что будет выполнено;
   - target;
   - риск/обратимость;
   - incident ID / guard hash, если применимо;
3. задаёт короткий вопрос на подтверждение;
4. пользователь отвечает естественно:
   - `да`;
   - `можно`;
   - `разрешаю`;
   - `можно clear block`;
   или эквивалентно;
5. после однозначного подтверждения агент сам выполняет служебную команду;
6. пользователь не должен копировать/набирать её вручную;
7. подтверждение действует только на конкретно описанную операцию;
8. новая операция/incident/hash требует нового подтверждения.

Для `safe clear-block` действует workflow по умолчанию:

- агент сначала делает read-only review incident;
- объясняет, почему block допустимо снять;
- спрашивает подтверждение конкретного incident;
- после `да` сам выполняет documented `safe clear-block`;
- проверяет `blocked=false`;
- продолжает task.

Нельзя clear:

- при неизвестном actual state;
- если recovery/rollback неоднозначен;
- если review требует undo/redo;
- только ради продолжения task без достаточного evidence.

Task card может заранее разрешить auto-clear только для узко описанного класса verifier false-negative; иначе default — запросить подтверждение.

Для Yandex Cloud применяется `docs/yc_agent_policy_ru.md`; дублировать там всю YC policy не нужно.
