...
### Guard approval: пользователь подтверждает, агент выполняет

Когда штатный `yc` из PATH возвращает необходимость:

```text
yc --guard approve <HASH>
```

агент:

1. не просит пользователя копировать/вводить approval-команду;
2. показывает:
   - исходную `yc` mutation;
   - точный target/resource;
   - существенные последствия, включая стоимость/риск;
   - guard hash;
3. спрашивает явное разрешение на конкретную mutation;
4. после однозначного `да` сам выполняет:

```text
yc --guard approve <HASH>
```

5. затем повторяет ровно исходную `yc`-команду один раз;
6. проверяет actual state;
7. approval не переносится на следующую mutation;
8. если hash изменился — нужно новое подтверждение;
9. если исходная команда после approval упала по другой причине — не запускать retry-loop/compensating mutation.

Обязательные ограничения:

- только `yc` из PATH;
- не обходить guard;
- не искать настоящий `yc.exe`;
- не менять PATH;
- не читать YC credentials/config;
- не скрывать от пользователя действие, которое он подтверждает.

Для high-risk/costly cloud mutation:

- требование отдельного explicit user authorization сохраняется;
- его можно совместить с guard-confirmation только если вопрос агента полностью и однозначно описывает конкретную high-risk mutation и последствия;
- общее `да` на предыдущую операцию не является разрешением следующей.
